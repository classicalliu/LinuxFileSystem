...
//	directory_set.erase(*directory_exist);
	// TODO 修复一个很奇葩的bug
	auto flag = std::find_if(directory_set.begin(), directory_set.end(), [directory_name](const std::shared_ptr<Directory> directory) {return directory->get_name() == directory_name; });
	directory_set.erase(*flag);
